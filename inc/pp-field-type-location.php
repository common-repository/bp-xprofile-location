	...
	class PP_Field_Location {
		...
		*/
		...
		function pp_loc_signup_usermeta( $meta ) {
			if ( isset( $meta['signup_profile_field_ids'] ) && !empty( $meta['signup_profile_field_ids'] ) ) {
				$profile_field_ids = explode(',', $meta['signup_profile_field_ids']);
				foreach ( $profile_field_ids as $field_id ) {
					$field = new BP_XProfile_Field( $field_id );
					if ($field->type == 'location' ) {
						if (isset($_POST['field_' . $field_id]) && ! empty( $_POST['field_' . $field_id] ) ) {
		...
		function pp_loc_signup_user( $user_id, $user_login, $user_password, $user_email, $usermeta ) {
			if ( ! is_multisite() ) {
				if ( $user_id ) {
					if ( bp_is_active( 'xprofile' ) ) {
						if ( isset( $usermeta['profile_field_ids'] ) && !empty( $usermeta['profile_field_ids'] ) ) {
							$profile_field_ids = explode(',', $usermeta['profile_field_ids']);
							foreach ($profile_field_ids as $field_id) {
								$field = new BP_XProfile_Field( $field_id );
								if ($field->type == 'location' ) {
									if (isset($usermeta['geocode_' . $field_id]) && ! empty( $usermeta['geocode_' . $field_id] ) ) {
